...
function authenticate(connection, payload) {
    if (payload.anon) {
        const username = _session2.default.getAnon(payload.session, payload.channel);
        ...
        _session2.default.get(payload.session, username => {
            if (!username) {
                // username not found
    ...
    connection.on('close', data => {
        if (connection.data.valid) {
            ...
            if (!ch.userCount(connection.data.username)) {
                ch.broadcast(createAction(TYPE.LEAVE, {
                    username: connection.data.username
                }));
            }
        }
